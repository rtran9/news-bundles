...
def get_captions_segments(media):
    ...
    for i in range(1,len(sorted_captions)):
        ...
        if idx>-1:
            segment_break_cpations.append(i)
    ...
    if len(commercial_captions)/((len(sorted_captions)+1)*1.0)>0.2 or len(stories)==0:
    ...
    if len(segment_break_cpations)==0:
        # this is not a news video! return no stories
        stories=None
